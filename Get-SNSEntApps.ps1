...
$entApps = Get-AzureADServicePrincipal -All $true    ##$entApp = $entApps[155] ##658 3 155 664    $entApp | FL *
$i = 0
...
foreach($entApp in $entApps){
    ...
    foreach($spOAuth2PermissionsGrant in $spOAuth2PermissionsGrants){ ## $spOAuth2PermissionsGrant = $spOAuth2PermissionsGrants[0]
        ...
        $permsFinal = "Ninguno"
            ...
            $permsFinal = $permsFinal + $newspOAuth2PermissionsGrant.'API Name'.ToString() + ', ' +  $newspOAuth2PermissionsGrant.'Claim value'.ToString() + "; "
            }
        }
        }

...
$entApps = Get-AzureADServicePrincipal -All $true    ##$entApp = $entApps[25] ##658 3 155 664    $entApp | FL *
$i = 0
...
foreach($entApp in $entApps){
    ...
    foreach($spOAuth2PermissionsGrant in $spOAuth2PermissionsGrants){ ## $spOAuth2PermissionsGrant = $spOAuth2PermissionsGrants[0]
        $permPid = Get-AzureADUser -ObjectId $spOAuth2PermissionsGrant.PrincipalId -ErrorAction SilentlyContinue -WarningAction SilentlyContinue
        $permPid.DisplayName 
...
$output | select * | Export-CSV -nti -Path "$((Get-Date).ToString('ddMMyyyy'))_PSEntAppsInventory.csv"